...
router.post("/register", async (req, res) => {
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      res.json({
        message:
          "Реєстрацію успішно! Перевірте email для підтвердження акаунту.",
      });
    } else {
      ...
      res.json({
        message:
          "Реєстрацію успішно! (Dev-режим: email підтверджено автоматично)",
        token,
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user.email_verified) {
      return res
        .status(403)
        .json({ error: "Підтвердіть email перед входом. Перевірте пошту." });
    }
...
router.get("/verify/:token", (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(400)
        .json({ error: "Невірний або прострочений токен верифікації" });
    ...
    addNotification(
      user.id,
      "Вітаємо в МатеМакс! Ваш акаунт підтверджено. 🎉",
      "success",
    );
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    if (!user)
      return res.json({
        message: "Якщо такий email існує — лист з інструкціями надіслано.",
      });
...
router.post("/reset-password", async (req, res) => {
  try {
    ...
    const user = db
      ...
      .get(token);
    if (!user)
      return res.status(400).json({ error: "Невірний токен скидання пароля" });
    if (user.reset_token_expires < Date.now()) {
      return res
        .status(400)
        .json({ error: "Посилання для скидання прострочене. Запросіть нове." });
    }
    ...
    res.json({
      message: "Пароль успішно змінено! Тепер увійдіть з новим паролем.",
    });
  } catch (e) {
...
router.post("/change-password", auth, async (req, res) => {
  try {
    ...
    if (!newPassword || newPassword.length < 6) {
      return res
        .status(400)
        .json({ error: "Новий пароль — мінімум 6 символів" });
    }
