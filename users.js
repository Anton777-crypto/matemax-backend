...
router.get('/my-children', auth, (req, res) => {
  try {
    const db = getDB();
    const children = db.prepare('SELECT * FROM users WHERE parent_id = ?').all(req.user.id);
    res.json({ children: children.map(formatUser) });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// ── POST /api/users/add-child ─────────────────────────────────
router.post('/add-child', auth, async (req, res) => {
  try {
    if (req.user.role !== 'parent') {
      return res.status(403).json({ error: 'Тільки для батьків' });
    }
    ...
    db.prepare(`
      INSERT INTO users (id, email, password_hash, name, role, email_verified, parent_id, subjects)
      VALUES (?, ?, ?, ?, 'student', 1, ?, ?)
    `).run(id, email.toLowerCase().trim(), password_hash, name.trim(), req.user.id, grade || null);

    const child = db.prepare('SELECT * FROM users WHERE id = ?').get(id);
    res.json({ child: formatUser(child) });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: e.message });
  }
